...
from util.box_ops import box_cxcywh_to_xyxy, generalized_box_iou, box_iou, generalized_box_iou_with_wasserstein
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        dist_x = torch.cdist(out_bbox[:, [0, 2]], tgt_bbox[:, [0, 2]], p=1)  # indices [0, 2] for x coordinates
        dist_y = torch.cdist(out_bbox[:, [1, 3]], tgt_bbox[:, [1, 3]], p=1)  # indices [1, 3] for y coordinates

        threshold1 = 1920 * 0.3
        mask1 = dist_x > threshold1
        cost_cenx = torch.zeros_like(dist_x)
        cost_cenx[mask1] = -1

        threshold2 = 1080 * 0.3
        mask2 = dist_y > threshold2
        cost_ceny = torch.zeros_like(dist_y)
        cost_ceny[mask2] = -1
        
        iou, union = box_iou(out_bbox, tgt_bbox)
        threshold3 = 0.6
        mask3 = iou <= threshold3
        cost_pos = torch.zeros_like(dist_x)
        cost_pos[mask3] = -1
        ...
        # Final cost matrix
        cost_RFD = -generalized_box_iou_with_wasserstein(box_cxcywh_to_xyxy(out_bbox), box_cxcywh_to_xyxy(tgt_bbox))
        C = self.a * cost_class - self.b * (cost_giou+ cost_pos + cost_cenx + cost_ceny) + self.c * cost_RFD
