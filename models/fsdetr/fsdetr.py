...
class FSDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        #HFF
        self.fs = FrequencySelection(in_channels=256)
        self.stride_level = add_conv(256, 256, 3, 2) 
        self.conv13 = nn.Conv2d(in_channels=768, out_channels=256, kernel_size=1, padding=0)
        self.conv12 = nn.Conv2d(in_channels=2048, out_channels=256, kernel_size=1, padding=0)
        self.conv11 = nn.Conv2d(in_channels=3584, out_channels=256, kernel_size=1, padding=0)
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        # HFF
        srcs[0] = self.fs(srcs[0])
        # Multi-scale feature integration(3)
        src31 = srcs[3] * self.stride_level(srcs[2]) 
        src21 = srcs[2] * self.stride_level(srcs[1]) * F.interpolate(srcs[3], size=srcs[2].shape[-2:])
        src11 = srcs[1] * F.interpolate(srcs[2], size=srcs[1].shape[-2:])
        src32 = torch.cat((src31, self.stride_level(src21)), dim=1)
        src22 = torch.cat((src21, self.stride_level(src11), F.interpolate(src32, size=src21.shape[-2:])), dim=1)
        src12 = torch.cat((src11, F.interpolate(src22, size=src11.shape[-2:])), dim=1)
        srcs[3] = torch.cat((srcs[3],src32),dim=1)
        srcs[2] = torch.cat((srcs[2], src22, F.interpolate(srcs[3], size=src22.shape[-2:])),dim=1)
        srcs[1] = torch.cat((srcs[1], src12, F.interpolate(srcs[2], size=src12.shape[-2:])),dim=1)
        srcs[3] = self.conv13(srcs[3])
        srcs[2] = self.conv12(srcs[2])
        srcs[1] = self.conv11(srcs[1])
...
@MODULE_BUILD_FUNCS.registe_with_name(module_name='fsdetr')
def build_fsdetr(args):
