...
class FrequencyAwareSelfAttentionBLC(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(FrequencyAwareSelfAttentionBLC, self).__init__()
        # 定义用于生成 Q, K, V 的三个 1x1 卷积
    ...
    def forward(self, FIn1_i):
        # 输入形状 (B, L, C)
        B, L, C = FIn1_i.shape
        
        # 调整形状为 (B, C, L)，以适配 1D 卷积
        FIn1_i = FIn1_i.permute(0, 2, 1)
        
        # 通过三个卷积生成 Query (Q), Key (K), Value (V)
        Q = self.query_conv(FIn1_i)  # 输出形状 (B, out_channels, L)
        K = self.key_conv(FIn1_i)   # 输出形状 (B, out_channels, L)
        V = self.value_conv(FIn1_i) # 输出形状 (B, out_channels, L)
        
        # 对 Q, K, V 应用快速傅里叶变换 (FFT)
        Qf = fft.fft(Q, dim=-1)  # 频域中的 Query
        Kf = fft.fft(K, dim=-1)  # 频域中的 Key
        Vf = fft.fft(V, dim=-1)  # 频域中的 Value
        
        # 计算注意力权重 (复数操作)
        Qf_real, Qf_imag = Qf.real, Qf.imag
        Kf_real, Kf_imag = Kf.real, Kf.imag
        
        # 实部和虚部计算
        Af_real = torch.softmax(Qf_real * Kf_real - Qf_imag * Kf_imag, dim=-1)
        Af_imag = torch.softmax(Qf_real * Kf_imag + Qf_imag * Kf_real, dim=-1)
        Af = Af_real + 1j * Af_imag  # 注意力图 (复数形式)
        
        # 重新校准 Value (Vf) 值
        recalibrated_Vf = Af * Vf
        
        # 对校准后的值执行逆快速傅里叶变换 (IFFT)
        Ffre_complex = fft.ifft(recalibrated_Vf, dim=-1)  # 复数输出
        Ffre_amplitude = torch.abs(Ffre_complex)         # 提取复数的幅度
        
        # 通过最终卷积调整输出特征
        Ffre = self.output_conv(Ffre_amplitude)  # 输出形状 (B, out_channels, L)
        
        # 调整回原始形状 (B, L, out_channels)
        Ffre = Ffre.permute(0, 2, 1)
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        #FS
        self.fs = FrequencyAwareSelfAttentionBLC(256,256)
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, key_padding_mask=None):
        ...
        s1 = self.fs(self.with_pos_embed(src, pos))
